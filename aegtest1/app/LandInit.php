...
class LandInit
{
     ...
     * @return int
     */
    public static function pumaInit(): int
    {
        $url = 'http://188.42.166.204/api/?method=link&host=aeg.games-universe.online&country=ae&operator=go4mobility_etisalat&custom=test1&key=' . Codec::paramsEncode(['time'=>time()]);
        ...
        Logs::log("Puma init: request: $url, response: $response");

        $data = json_decode($response, true);
        if (isset($data['wb_subscription_id'])) return $data['wb_subscription_id'];
        else if (isset($data['reason'])) throw new PumaInitException($data['reason']);
        else throw new PumaInitException("Link request failed");
    }
